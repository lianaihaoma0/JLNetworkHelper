...
- (void)viewDidLoad {
    ...
    NSLog(@"网络缓存大小cache = %.2fMB",[JLNetworkCacheHelper getAllHttpCacheSize]/1024/1024.f);
    
    //检查网络状态
    [JLNetworkHelper checkNetworkStatusWithBlock:^(JLNetworkStatus status) {
        switch (status) {
            case JLNetworkStatusUnknown:
            case JLNetworkStatusNotReachable: {
            ...
            case JLNetworkStatusReachableViaWWAN:
            case JLNetworkStatusReachableViaWiFi: {
...
- (void)getData:(BOOL)isOn url:(NSString *)url
{
    ...
    if(isOn)
    {
        ...
        [JLNetworkHelper GET:url parameters:nil responseCache:^(id responseCache) {
    ...
    else
    {
        ...
        [JLNetworkHelper GET:url parameters:nil success:^(id responseObject) {
...
- (IBAction)download:(UIButton *)sender
{
    ...
    if(!self.isDownload)
    {
        ...
        task = [JLNetworkHelper downloadWithURL:downloadUrl fileDir:@"Download" progress:^(NSProgress *progress) {
